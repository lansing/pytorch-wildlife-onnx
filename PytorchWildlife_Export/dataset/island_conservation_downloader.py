"""
island_conservation_downloader.py
----------------------------------
Downloads Island Conservation camera trap images and annotations from LILA.

Data source
    LILA Island Conservation Camera Traps
    https://lila.science/datasets/island-conservation-camera-traps

    Annotation ZIP:
        https://lilawildlife.blob.core.windows.net/lila-wildlife/island-conservation/
            island-conservation-camera-traps.json.zip

    Images:
        https://lilawildlife.blob.core.windows.net/lila-wildlife/island-conservation/{file_name}

Category mapping
    Island Conservation has species like cats, rats, rabbits, native birds.
    Uses wcs_category_to_md_class on category names.
    Unrecognized wildlife → animal (0).
"""
...
ANNOTATION_URL = (
    "https://storage.googleapis.com/public-datasets-lila/islandconservationcameratraps/"
    "island_conservation_camera_traps_1.02.zip"
)
IMAGE_BASE_URL = (
    "https://lilawildlife.blob.core.windows.net/lila-wildlife/islandconservationcameratraps/public"
)
DEFAULT_CACHE_DIR = Path.home() / ".cache" / "pytorch_wildlife_export" / "island_conservation"
ANNOTATION_FILENAME = "island_conservation_camera_traps_1.02.json"
...
def _build_category_map(categories: list[dict]) -> dict[int, int | None]:
    """Map Island Conservation category_id → MD class.

    Uses wcs_category_to_md_class; result of None stays None (skip),
    all wildlife that doesn't match person/vehicle terms → 0 (animal).
    """
...
def _download_annotations(cache_dir: Path) -> Path:
    ...
    try:
        ...
        LOGGER.error(
            "Failed to download Island Conservation annotations.\n%s\n"
            "Check https://lila.science/datasets/island-conservation-camera-traps "
            "for the current URL.\nIsland Conservation will be skipped.",
            exc,
        )
        raise
...
def _build_label_files(
    ...
    for img in selected_images:
        ...
        location_id = f"{SOURCE}:{location}" if location is not None else None
    ...
    LOGGER.info(
        "Island Conservation labels written: %d images  "
...
def download_and_convert_island_conservation(
    output_dir: Path,
    max_animal: int = 500,
    max_empty: int = 150,
    seed: int = 42,
    ...
    """Full Island Conservation pipeline.

    Returns list of records with keys:
        image_path, label_path, source="ict", location_id, empty
    """
    ...
    try:
        annotation_path = _download_annotations(cache_dir)
    except RuntimeError:
        return []

    LOGGER.info("Parsing Island Conservation annotation JSON: %s", annotation_path)
    with open(annotation_path) as f:
        data = json.load(f)

    cat_map = _build_category_map(data["categories"])
    ...
    LOGGER.info(
        "Island Conservation images with ≥1 animal annotation: %d  (requesting %d)",
    ...
    for img in selected_images:
        ...
        local_name = f"{SOURCE}_{stem}.jpg"
    ...
    if max_empty > 0:
        ...
        LOGGER.info("Sampling %d empty Island Conservation images.", len(empty_imgs))
    ...
    LOGGER.info("Island Conservation dataset ready: %d records.", len(records))
    return records
