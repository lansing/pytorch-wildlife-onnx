...
class ONNXExporter(BaseONNXExporter):
    ...
    def export(
        ...
        **kwargs,
        ...
        if len(processed_dummy_input) == 1 and not isinstance(dummy_input, tuple):
            ...
            processed_dummy_input = tuple(
                processed_dummy_input
            )  # Ensure it's a tuple if multiple inputs
        ...
        temp_float_onnx_path = (
            output_path.replace(".onnx", "_float.onnx")
        ...
        try:
            ...
            return self._quantize_and_simplify_model(
                ...
                input_shape=processed_dummy_input[0].shape
                if isinstance(processed_dummy_input, tuple)
                else processed_dummy_input.shape,  # Pass first dummy input shape for calibration
            )
    ...
    def _quantize_and_simplify_model(
        ...
        if export_format in ["int8", "uint8"]:
            ...
            quant_type = (
                QuantType.QInt8 if export_format == "int8" else QuantType.QUInt8
            )
            activation_type = (
                QuantType.QInt8 if export_format == "int8" else QuantType.QUInt8
            )  # Set activation_type based on export_format
