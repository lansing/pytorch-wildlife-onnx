...
YOLOV10_COMPATIBLE_ONNX_PATH = os.path.join(OUTPUT_DIR, f"{YOLOV10_COMPATIBLE_VERSION}_1280x1280_v9_compatible.onnx")
YOLOV10_ORIGINAL_ONNX_PATH = os.path.join(OUTPUT_DIR, f"{YOLOV10_COMPATIBLE_VERSION}_1280x1280_raw.onnx")
...
def run_demo():
    ...
    inference_session = ONNXInferenceSession(onnx_model_path=YOLOV10_COMPATIBLE_ONNX_PATH)
    ...
    output_image_custom_path = os.path.join(OUTPUT_DIR, f'detected_sample_image_{YOLOV10_COMPATIBLE_VERSION}_v9_compatible_custom_pp.jpg')
    ...
    visualize_detections(SAMPLE_IMAGE_PATH, custom_detections, output_image_custom_path, GLOBAL_CLASS_NAMES, title=f"V10 (v9-comp) Custom PP")
    ...
    if not os.path.exists(YOLOV10_ORIGINAL_ONNX_PATH):
        ...
        export_tool_args = [
            "export_tool.py",
            "--model_type", "yolov9", # Use yolov9 model_type for original YOLOv10 export
            "--model_version", YOLOV10_COMPATIBLE_VERSION,
    ...
    output_image_baseline_path = os.path.join(OUTPUT_DIR, f'detected_sample_image_{YOLOV10_COMPATIBLE_VERSION}_original_ultralytics_baseline.jpg')
    ...
    visualize_detections(SAMPLE_IMAGE_PATH, ultralytics_detections, output_image_baseline_path, GLOBAL_CLASS_NAMES, title=f"UL Baseline: {YOLOV10_COMPATIBLE_VERSION} Original")

    print("\n--- Comparison ---")
    print(f"Custom PP (V10 compatible) Detections: {len(custom_detections)}")
    print(f"Ultralytics Baseline (V10 Original) Detections: {len(ultralytics_detections)}")
