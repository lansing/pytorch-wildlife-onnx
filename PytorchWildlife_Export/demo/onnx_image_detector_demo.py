...
def run_demo(model_paths: List[str]):
    ...
    for model_path in model_paths:
        ...
        visualize_detections(SAMPLE_IMAGE_PATH, custom_detections, output_image_custom_path, GLOBAL_CLASS_NAMES, title=f"Custom PP: {model_filename}")
        ...
        if "raw" in model_filename or "nms" in model_filename: # Heuristic to check if it's our exported model
            ...
            visualize_detections(SAMPLE_IMAGE_PATH, ultralytics_detections, output_image_baseline_path, GLOBAL_CLASS_NAMES, title=f"UL Baseline: {model_filename}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run ONNX image detector demo for one or more models.")
    parser.add_argument(
        "--model_path",
        type=str,
