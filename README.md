...
- **Blanket quantization profile (default):** The `blanket` profile wraps Conv, residual `Add`, and `MaxPool` nodes in INT8 QDQ (Quantize/Dequantize) pairs. This eliminates layout-reformat overhead on the shortcut paths in YOLOv10's C2f blocks and has been observed to outperform the alternative `conv` profile on all tested NVIDIA hardware. The `conv` profile (Conv nodes only) is retained as a `--quant_profile conv` CLI option for experimentation, but is not recommended.
